...
<header class="flex items-center justify-between sticky top-0 z-50 bg-gray-100 py-4 px-20">
    <h1 class="text-3xl font-semibold">Create New Object</h1>
...
<div class="container h-screen flex flex-col mx-auto px-4 py-8">
    <form id="aoqrForm" action="{{ route('admin.aoqr_objects.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
...
{{-- <!-- Image Upload (Max 5) -->
        <div class="mb-4">
            <label for="images" class="block text-lg font-medium">Upload Images (Max 5)</label>
            <input type="file" name="images[]" class="w-full px-4 py-2 border border-gray-300 rounded-md mb-2" accept="image/*">
            <input type="file" name="images[]" class="w-full px-4 py-2 border border-gray-300 rounded-md mb-2" accept="image/*">
            <input type="file" name="images[]" class="w-full px-4 py-2 border border-gray-300 rounded-md mb-2" accept="image/*">
            <input type="file" name="images[]" class="w-full px-4 py-2 border border-gray-300 rounded-md mb-2" accept="image/*">
            <input type="file" name="images[]" class="w-full px-4 py-2 border border-gray-300 rounded-md mb-2" accept="image/*">
...
<div class="mb-4">
    ...
    <input type="text" name="name_object" class="w-full px-4 py-2 border border-gray-300 rounded-md" required>
</div>

<!-- Location (object) -->
<div class="mb-4">
    <label for="location_object" class="block text-lg font-medium">Lokasi (Object)</label>
    <input type="text" name="location_object" class="w-full px-4 py-2 border border-gray-300 rounded-md" required>
</div>

<!-- Description (English) -->
<div class="mb-4">
    <label for="description_object" class="block text-lg font-medium">Deskripsi (Object)</label>
    <textarea name="description_object" class="w-full px-4 py-2 border border-gray-300 rounded-md" required></textarea>
</div>
